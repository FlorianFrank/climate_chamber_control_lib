...
bool ClimateChamberWrapper::Initialize(const std::string &ipAddr, uint16_t port, uint8_t channel)
{
    m_channel = channel;
...
bool ClimateChamberWrapper::GetErrorCode(int *errCodeRet) // TODO don't use ugly int pointers
{
...
/*static*/ bool ClimateChamberWrapper::LogMessageAndReturn(bool returnValue, Level level, const char* fileName, unsigned int lineNumber, const char* message, ...)
{
    va_list vaList;
